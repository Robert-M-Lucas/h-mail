use crate::root::receiving::interface::get_user_pow_policy::{UserPowPolicyRequest, UserPowPolicyResponse};
...
pub async fn get_user_pow_policy(
    ...
    let policy = DB.lock().await.as_ref().unwrap().get_user_pow_policy(pow_policy_request.destination());
