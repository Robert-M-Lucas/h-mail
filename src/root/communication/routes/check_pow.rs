...
pub async fn check_pow(Query(pow_request): Query<CheckPow>) -> (StatusCode, Json<CheckPowStatus>) {
    let Ok(token) = base64_to_big_uint(pow_request.token()) else {
        return (StatusCode::BAD_REQUEST, CheckPowStatus::Failure(PowFailureReason::BadRequestCanRetry).into());
    };
    ...
    match result {
        ...
        Err(e) => (StatusCode::EXPECTATION_FAILED, CheckPowStatus::Failure(e).into()),
    }
}
